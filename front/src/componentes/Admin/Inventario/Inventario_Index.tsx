...
const Inventario_Index: React.FC = () => {
  ...
  // Detecta si la pantalla es móvil para mostrar cards
  useEffect(() => {
    const handleResize = () => {
      ...
      {isMobile ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {filteredProductos.length === 0 && !loading && (
            <Typography.Text>No hay productos disponibles</Typography.Text>
          )}
          {filteredProductos.map(producto => (
            ...
            >
              ...
              />
              <h3 style={{ marginTop: 12 }}>{producto.nombre}</h3>
              <p><b>Marca:</b> {producto.marca?.descripcion || 'N/A'}</p>
              <p><b>Categoría:</b> {producto.categoria?.descripcion || 'N/A'}</p>
              <p>
                <b>Estado:</b> 
                <Tag color={producto.estado === 'activo' ? 'green' : 'red'}>
              ...
              <p>
                <b>Stock:</b> 
                <span style={{
                  color: Number(producto.stock) <= 10 ? 'red' : Number(producto.stock) <= 20 ? 'orange' : 'green',
                  fontWeight: 'bold'
                }}>
                  {producto.stock}
                </span>
              </p>
              <p><b>Precio Minorista:</b> ${producto.precio_minorista?.toFixed(2) || '0.00'}</p>
              <p><b>Precio Mayorista:</b> ${producto.precio_mayorista?.toFixed(2) || '0.00'}</p>
              <Space>
                <Button size="small" onClick={() => handleEdit(producto)}>Editar</Button>
                <Popconfirm
                  title="¿Estás seguro de eliminar este producto?"
                  description={`Producto: ${producto.nombre}`}
                  onConfirm={() => handleDelete(producto.id_producto)}
                  okText="Sí, eliminar"
                  cancelText="Cancelar"
