...
def obtener_usuario(identificacion: str, db: Session = Depends(get_db)):
...
def editar_usuario(identificacion: str, usuario: dict = Body(...), db: Session = Depends(get_db)):
    return usuarios_controller.update_usuario(db, identificacion, usuario)

@router.delete("/usuarios/{identificacion}")
def eliminar_usuario(identificacion: str, db: Session = Depends(get_db)):
